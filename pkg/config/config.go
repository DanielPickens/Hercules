...
// Config tracks Hercules configuration options.
type Config struct {
	Hercules  `yaml:"hercules"`
	Flags     *Flags
	LintLevel int
}

// NewConfig create a new Hercules configuration.
func NewConfig(flags *Flags) (*Config, error) {
	cfg := Config{Hercules: NewHercules()}

	if isSet(flags.vegetable) {
		f, err := ioutil.ReadFile(*flags.vegetable)
		if err != nil {
			return nil, err
		}
		if err := yaml.Unmarshal(f, &cfg); err != nil {
			return nil, fmt.Errorf("Invalid veg config file -- %w", err)
		}
	}
	cfg.Flags = flags
