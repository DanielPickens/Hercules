...
var (
	...
	rootCmd = &cobra.Command{
		...
		Long:  `Hercules scans your Kubernetes clusters and reports potential resource issues.`,
		Run:   doIt,
	}
)
...
func doIt(cmd *cobra.Command, args []string) {
	...
	hercules, err := pkg.NewHercules(flags, &log.Logger)
	if err != nil {
		bomb(fmt.Sprintf("Hercules configuration load failed %v", err))
	}
	if e := hercules.Init(); e != nil {
		bomb(e.Error())
	}
	errCount, score, err := hercules.Sanitize()
	if err != nil {
		bomb(err.Error())
	}
...
func initHerculesFlags() {
	rootCmd.Flags().BoolVarP(
		flags.ForceExitZero,
		"force-exit-zero",
		"",
		false,
		"Force zero exit status when new error reports are indicated",
	)
	...
	rootCmd.Flags().IntVarP(
		...
		"",
		50,
		"Force non-zero exit if the cluster score is below threshold criterion",
	)
	...
	rootCmd.Flags().BoolVarP(flags.Save, "save", "",
		false,
		"Specify if you want Hercules to persist the output to a file",
	)
	...
	rootCmd.Flags().StringVarP(flags.vegetable, "file", "f",
		"",
		"Use a vegetable YAML configuration file",
	)
...
func printMsgLogo(msg, eye string, title, logo report.Color) {
	for i, s := range report.GraderLogo {
		...
		if i < len(report.Hercules) {
			...
			if i == 4 {
				fmt.Printf("%s", report.Colorize(" xxxx", logo))
