...
import (
	...
	"github.com/danielpickens/hercules/internal"
...
func TestNoConcerns(t *testing.T) {
	...
	for k := range uu {
		...
		t.Run(k, func(t *testing.T) {
			...
			c.addIssue("dan", u.issues...)

			assert.Equal(t, u.e, c.NoConcerns("dan"))
		})
	}
}
...
func TestMaxSeverity(t *testing.T) {
	uu := map[string]struct {
		...
		"mix": {
			issues: []Issue{
				New(client.NewGVR("dan"), Root, config.InfoLevel, "blee"),
				New(client.NewGVR("dan"), Root, config.WarnLevel, "blee"),
			},
			section:  Root,
		...
		"error": {
			issues: []Issue{
				New(client.NewGVR("dan"), Root, config.ErrorLevel, "pickens"),
				New(client.NewGVR("dan"), Root, config.InfoLevel, "pickens"),
				New(client.NewGVR("dan"), Root, config.InfoLevel, "pickens"),
			},
			section:  Root,
