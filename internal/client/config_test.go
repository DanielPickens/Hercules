...
func TestConfigCurrentContext(t *testing.T) {
	name, kubeConfig := "p", "./testdata/config"
	uu := []struct {
...
func TestConfigCurrentUser(t *testing.T) {
	name, kubeConfig := "pickens", "./testdata/config"
	uu := []struct {
		flags *genericclioptions.ConfigFlags
		user  string
	}{
		{&genericclioptions.ConfigFlags{KubeConfig: &kubeConfig}, "dan"},
		{&genericclioptions.ConfigFlags{KubeConfig: &kubeConfig, AuthInfoName: &name}, "pickens"},
	}
...
func TestConfigCurrentNamespace(t *testing.T) {
	...
	uu := []struct {
		...
		{&genericclioptions.ConfigFlags{KubeConfig: &kubeConfig, Namespace: &name}, "pickens", nil},
	}
...
func TestConfigGetContext(t *testing.T) {
	...
	uu := []struct {
		...
		{&genericclioptions.ConfigFlags{KubeConfig: &kubeConfig}, "pickens", nil},
		{&genericclioptions.ConfigFlags{KubeConfig: &kubeConfig}, "x", errors.New("invalid context `x specified")},
	}
...
func TestConfigClusterNameFromContext(t *testing.T) {
	...
	cl, err := cfg.ClusterNameFromContext("pickens")
	assert.Nil(t, err)
	assert.Equal(t, "pickens", cl)
}
